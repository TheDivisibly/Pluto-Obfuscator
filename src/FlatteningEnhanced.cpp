...
Function *MyFlatten::buildUpdateKeyFunc(Module *m) {
    ...
    Value *pos = irb.CreateLoad(irb.getInt32Ty(), irb.CreateGEP(
        irb.getInt32Ty(), posArray, irb.CreateLoad(irb.getInt32Ty(), i)));
...
void MyFlatten::DoFlatten(Function *f, int seed, Function *updateFunc) {
    ...
    for (std::vector<BasicBlock *>::iterator b = origBB.begin();
        ...
        Value *visited = irb.CreateLoad(irb.getInt32Ty(), ptr);
        if (doms.size() != 0) {
        ...
        irb.CreateCall(
            ...
            {visited, irb.getInt32(doms.size()),
                        irb.CreateGEP(irb.getInt32Ty(), dom_variable,
...
PreservedAnalyses MyFlattenPass::run(Module &M, ModuleAnalysisManager &AM) {
  MyFlatten flatten(1);
  flatten.runOnModule(M);
